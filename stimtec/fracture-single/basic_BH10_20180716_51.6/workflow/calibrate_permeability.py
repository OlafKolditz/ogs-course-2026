"""
Kalibriert einen einzelnen Skalierungsfaktor S fuer die Permeabilitaetsfunktion
kappa_temporal in simulation/stimtec-square-expression.prj, sodass die an
x,y=(0,0) simulierte Druckaufbaukurve moeglichst gut mit der digitalisierten,
gemessenen BH10-Kurve uebereinstimmt.

Die Kurvenform (Zeitfenster/Stufen, Verhaeltnis k_max/k_min) bleibt erhalten;
es wird nur ein globaler Faktor S auf k_min=5e-11 und k_max=1e-9 angewendet:
    k_min(S) = S * 5e-11
    k_max(S) = S * 1e-9

Fuer jeden Kandidaten S wird:
  1. der <parameter name="kappa_temporal"> Block in der .prj-Datei neu geschrieben
  2. OGS ausgefuehrt
...
# Basis-Permeabilitaetswerte (aktueller Stand vor der Kalibrierung)
BASE_K_MIN = 5e-11
BASE_K_MAX = 1e-9
...
def build_kappa_expression(k_min, k_max):
    """Baut die verschachtelte ?:-Expression fuer kappa_temporal fuer gegebene k_min/k_max."""
    k_values = [
        k_min + (v - V_MIN) / (V_MAX - V_MIN) * (k_max - k_min) for v in VALUES
    ]
    ...
    for i in range(n - 1):
        ...
        if i == 0:
            lines.append(f"(t &lt; {t2} ? {k1:.6e} :")
        elif k1 == k2:
            lines.append(f" t &lt; {t2} ? {k1:.6e} :")
        else:
            lines.append(
                f" t &lt; {t2} ? {k1:.6e} + ({k2:.6e} - {k1:.6e}) / ({t2} - {t1}) * (t - {t1}) :"
            )
    lines.append(f" {k_values[-1]:.6e})")
    return "\n                ".join(lines)


def write_kappa_block(k_min, k_max):
    with open(PRJ_PATH, "r", encoding="ISO-8859-1") as f:
        content = f.read()

    expression = build_kappa_expression(k_min, k_max)
    factor = k_min / BASE_K_MIN

    new_block = f"""<parameter>
            <name>kappa_temporal</name>
            <type>Function</type>
            <!-- Kalibrierter Skalierungsfaktor S={factor:.6g} auf Basis-Stufenfunktion
                 (BASE_K_MIN={BASE_K_MIN:.3e}, BASE_K_MAX={BASE_K_MAX:.3e}):
                 k_min = {k_min:.6e}, k_max = {k_max:.6e} -->
            <expression>
                {expression}
            </expression>
        </parameter>"""
    ...
    pattern = re.compile(
        r"<parameter>\s*<name>kappa_temporal</name>.*?</parameter>", re.DOTALL
    )
    content_new, count = pattern.subn(new_block, content)
    if count != 1:
        raise RuntimeError(f"Erwartet genau 1 kappa_temporal-Block, gefunden: {count}")
...
def objective(log10_S):
    ...
    _eval_log.append((S, k_min, k_max, err))
    print(f"S={S:8.4f}  k_min={k_min:.3e}  k_max={k_max:.3e}  RMSE={err:.4e} Pa")
    return err


def main():
    print("Starte Kalibrierung des Permeabilitaets-Skalierungsfaktors S ...")
    print(f"Basis: k_min={BASE_K_MIN:.3e}, k_max={BASE_K_MAX:.3e} (S=1)\n")

    res = minimize_scalar(
        objective, bounds=(-1.0, 2.0), method="bounded",
        options={"xatol": 0.03, "maxiter": 25},
    )

    best_log10_S = res.x
    best_S = 10 ** best_log10_S
    best_k_min = best_S * BASE_K_MIN
    best_k_max = best_S * BASE_K_MAX

    print("\n--- Kalibrierung abgeschlossen ---")
    print(f"Bestes S = {best_S:.4f}")
    print(f"k_min = {best_k_min:.4e} m^2, k_max = {best_k_max:.4e} m^2")
    print(f"RMSE  = {res.fun:.4e} Pa")

    print("\nAlle getesteten Kandidaten:")
    for S, k_min, k_max, err in sorted(_eval_log, key=lambda e: e[0]):
        print(f"  S={S:8.4f}  k_min={k_min:.3e}  k_max={k_max:.3e}  RMSE={err:.4e}")
    ...
    write_kappa_block(best_k_min, best_k_max)
    run_ogs()
